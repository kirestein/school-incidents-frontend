...
const AdvertenciasComponents: React.FC = () => {

  const { state, getAlunosAdvertidos } = useMyContext();

  useEffect(() => {
    if (state.alunosAdvertidos === null) {
      getAlunosAdvertidos();
    }
  }, [state.alunosAdvertidos, getAlunosAdvertidos]);

  const data = state.alunosAdvertidos;
  return (
    <C.Container>
